    ...
    private fun initStoresModule() {
        ...
        btnGetCoords.setOnClickListener {
            ...
            try {
                ...
                // Lấy tối đa 3 kết quả để tìm kết quả chính xác nhất
                ...
                if (addresses != null && addresses.isNotEmpty()) {
                    // Ưu tiên kết quả đầu tiên (thường là chính xác nhất từ Google)
                    val location = addresses[0]
                    latInput.setText("%.6f".format(Locale.US, location.latitude))
                    lngInput.setText("%.6f".format(Locale.US, location.longitude))
                    
                    val fullAddress = location.getAddressLine(0)
                    Toast.makeText(this, "Đã khớp: $fullAddress", Toast.LENGTH_LONG).show()
                } else {
                    Toast.makeText(this, "Không tìm thấy tọa độ. Hãy thử thêm Tên đường, Phường, Quận, Thành phố để chính xác hơn.", Toast.LENGTH_LONG).show()
                }
            } catch (e: Exception) {
                btnGetCoords.text = "Tìm tọa độ từ địa chỉ trên"
                btnGetCoords.isEnabled = true
